...
<div class="col-12 grid-margin stretch-card">
  <div class="card">
    <div class="card-body">
      <h4 class="card-title">Laporan Absensi Bulan({{\Carbon\Carbon::parse($cari)->isoFormat('MMMM')}})</h4>
        ...
        <div class="container-fluid mt-4 mb-4">
            <div class="row mt-4">
                <div class="col d-inline mb-4">
                    <form class="d-flex" style="width: 60%" method="POST" action="/laporan-presensi-bulanan">
                        @csrf
                ...
                <div class="col">
                    @if ($data->count() >= 1)
                    <a href="/laporan-presensi-bulanan/print/{{$cari}}" target="__blank" class="btn btn-dark btn-sm rounded-1  btn-icon-text" style=" float:right " >
        ...
        <table class="table select-table table-striped table-hover border " id="example" >
          <thead class="thead-dark" >
            <tr>
              ...
              <th class="text-center text-light" >Absensi Masuk</th>
              <th class="text-center text-light" >Absensi Keluar</th>
...
@if ($item->absensi_masuk == null)
              ...
              @elseif (\Carbon\Carbon::parse($item->absensi_masuk)->format('H:i') >= $pengaturan->waktu_masuk)
                @php
                    $absmasuk=\Carbon\Carbon::parse($item->absensi_masuk)->format('H:i');
                    @endphp
                    <td class="text-center" ><div class="badge badge-opacity-warning bg-warning text-light ">Terlambat {{\Carbon\Carbon::parse($absmasuk)->diffForHumans($pengaturan->waktu_masuk) }} </div></td>
              @else
              <td class="text-center" ><div class="badge badge-opacity-danger bg-success text-light "> Berhasil Absensi </div></td>
              @endif --}}
